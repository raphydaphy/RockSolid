...
With all the technology you have constructed so far, you can now begin to work towards Rockets. They are used to send Drills and Satellites into space, to collect information and mine Asteroids for Resources. Rockets are very complex, and require many things to start using them.

To get your first Rocket launched and into Space, you will need to start with a Refinery. Construct the Refinery in a Construction Table, and pump Oil into the bottom left side using a Fluid Conduit set to Output mode. You can supply Energy from any side of the machine with an Energy Conduit on Output mode, and use another Fluid Conduit to extract the produced Refined Fuel from the bottom right side. The Refined Fuel can be stored in buckets or a Fluid Tank.

Next, you will need a Tank, a Rocket Engine and a Rocket Fairing. Construct all three using a Construction Table. The parts can then be assembled into a full Rocket, again in the Construction Table. The Rocket can be placed into the world, taking up an area of 1x4 meters. Pump Refined Fuel into the Rocket using some Fluid Conduits set to Output mode, but don't launch it yet. When you do need to launch it, however, simply right-click the Rocket to open the GUI, and press the big Launch button.

Before launch, you will need to tell the Rocket what you want it to do. Sending an empty rocket is going to be pretty useless. Construct a Satelite Core as well as an Asteroid Data Chip. Put the Satelite Core in the top left slot of the Rocket, and the Asteroid Data Chip in the slot next to it, like in the image below. If everything is in the correct place and you have plenty of fuel in the Rocket, press Launch and watch it travel into space.

![A Rocket ready to launch](http://i.imgur.com/pykUsEM.png)

You will get notified when your rocket leaves the atmosphere, and when it begins to come back. Don't walk too far away from the launch site until it returns, or you might leave it in an unloaded chunk where it will not come back until you return to the area. When your rocket does eventually return, it should land right back where you launched it. If it falls out of the sky, it has ran out of fuel. In this case you will not get anything back except the Rocket itself.

Your recovered Rocket should contain the Satellite Core like when you sent it off, but now when you hover over the Asteroid Data Chip, it should contain an identification number. In order to learn more about the discovered Asteroid, construct an Analyzer and place the Data Chip into the input slot of the GUI. It will take a short moment to research your data chip, and eventually give you back a more informative Asteroid Data Chip with the Size and Primary Resource of your Asteroid now visible.

Now you can actually obtain resources from the Asteroid. Construct a Drill Core and replace the Satelite Core in your rocket with it. Put the analyzed Asteroid Data Chip in the Data Chip slot, ensure the rocket has enough fuel, and launch it back into space. It will take a little longer this time, and when it eventually returns it should be full of ore from the Asteroid.

Note that this ore is in its raw form, and therefore you would normally have to place it in the world and break it with a Pickaxe to obtain the Clusters, in order to produce Ingots. However, you can now construct a Rock Crusher to do this for you, and additionally double the cluster output as a bonus since it requires Energy to run and is far more precise than a crude Pickaxe. Your Asteroid Data Chip will be missing when the rocket returns with ore, but it is easy to create a new one, send it in a satellite and then in a mining rocket again to get more resources. The only limit at this point is your supply of Asteroid Data Chips and Refined Fuel.

You might notice that your Rocket returned with not just the Ore that you were able to obtain from Earth, but also potentially some Cobalt, Nickel and Aluminum ore. This will prove very useful later, but for now you can just use the Nickel to get Nickel Ingots much easier than separating Iron Clusters like you had to earlier.