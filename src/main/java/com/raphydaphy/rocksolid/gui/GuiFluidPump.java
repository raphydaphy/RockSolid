...
public class GuiFluidPump extends GuiContainer
{
    ...
    public void initGui(final IGameInstance game) {
        super.initGui(game);
        this.components.add(new ComponentProgressBar(this, this.guiLeft + 60, this.guiTop + 25, 80, 10, new Color(148,0,211), false, this.tile::getEnergyFullness));
        this.components.add(new ComponentProgressBar(this, this.guiLeft + 60, this.guiTop + 40, 80, 10, RockSolidLib.getFluidColor(tile.getFluidType()), false, this.tile::getFluidTankFullness));
    }
	...
	public void renderOverlay(IGameInstance game, IAssetManager manager, Graphics g)
	{
		...
		boolean mouseOverPowerBarY = (game.getMouseInGuiY() >= this.guiTop + 25) && (game.getMouseInGuiY() <= (this.guiTop + 25 + 10));
		...
		boolean mouseOverFluidBarY = (game.getMouseInGuiY() >= this.guiTop + 40) && (game.getMouseInGuiY() <= (this.guiTop + 40 + 10));
		
		if (mouseOverFluidBarX && mouseOverFluidBarY)
		{
			RockBottomAPI.getApiHandler().drawHoverInfoAtMouse(game, manager, g, false, 500, new String[]{"Storing " + this.tile.getCurrentFluid() + "mL of Fluid", "Produces 1000mL per operation"});
		}
	}
