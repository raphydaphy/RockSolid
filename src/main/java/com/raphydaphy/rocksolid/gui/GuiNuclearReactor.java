...
public class GuiNuclearReactor extends GuiContainer
{
	...
	public void renderOverlay(IGameInstance game, IAssetManager manager, Graphics g)
	{
		...
		boolean mouseOverPowerBarY = (game.getMouseInGuiY() >= this.guiTop + 10) && (game.getMouseInGuiY() <= (this.guiTop + 10 + 10));
		
		if (mouseOverPowerBarX && mouseOverPowerBarY)
		{
			RockBottomAPI.getApiHandler().drawHoverInfoAtMouse(game, manager, g, false, 500, new String[]{"Storing " + this.tile.getCurrentEnergy() + "kWh of Energy", "Produces 150kWh per tick"});
		}
	}
