...
public class TileElectricSeparator extends MultiTile
{
	...
	public TileEntity provideTileEntity(IWorld world, int x, int y, TileLayer layer)
	{
		return layer == TileLayer.MAIN && this.isMainPos(x, y, world.getState(x, y))
				? new TileEntityElectricSeparator(world, x, y)
				: null;
	}
			...
			AbstractEntityPlayer player)
	{
		...
		final TileEntityElectricSeparator tile = world.getTileEntity(main.getX(), main.getY(),
				TileEntityElectricSeparator.class);
		if (tile != null)
		{
			player.openGuiContainer(new GuiBasicPowered(player, tile, new Pos2(70, 15)),
					new ContainerElectricSeparator(player, tile));
			return true;
		}
		return false;
	}
