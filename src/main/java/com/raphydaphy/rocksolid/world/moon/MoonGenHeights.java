...
public class MoonGenHeights implements IMoonGenerator
{
	public static final ResourceName a = RockSolid.createRes("heights");
	public INoiseGen b;

	public MoonGenHeights()
	{
	}

	public static int estimate(TileLayer var0, int x, INoiseGen var2, int min, int max)
	{
		...
		int height = (int) (var8 * 1.8D * var8 * var8 * var8 * var8 * var8 * 45.0D);
		var8 = var2.make2dNoise((double) x / 100.0D, 0.0D) + var2.make2dNoise((double) x / 20.0D, 0.0D) * 2D;
		height = Math.min(max, Math.max(min, height + (int) (var8 / 3.5D * 20.0D)));
		if (var0 == TileLayer.BACKGROUND)
		{
			height -= Util.ceil(var2.make2dNoise((double) x / 10.0D, 0.0D) * 3.0D);
		}
