		...
		e.registerListener(EntityTickEvent.class, (result, event) -> {
			...
			if (event.entity instanceof AbstractEntityPlayer)
			{
										...
										if (lantern.getItem() instanceof ItemLantern)
										{
											if (lanternData.getInt("itemFuel") == 0)
											{
												if (data.getDataSet("accessory3") != null)
												{
													...
													{
														if (data.getDataSet("accessory3").getString("item_name")
																.equals("") == false)
														{
															ItemInstance accessory3 = ItemInstance
																	.load(data.getDataSet("accessory3"));
															if (accessory3 != null)
															{
																if (accessory3.getAmount() > 0)
																{
																	...
																	if (accessory3.getAmount() == 0)
																	{
																		...
																		RockBottomAPI.getNet()
																				.sendToServer(new PacketItemUpdate(
																						player.getUniqueId(),
																						"accessory3", new DataSet()));
																	} else
																	{
				...
				if (jetpack != null)
				{
					...
					if (engineActive)
					{
						...
						if (ModKeybinds.keyJetpackHover.isPressed()
								&& RockBottomAPI.getGame().getGuiManager().getGui() == null)
						{
					...
					if (jetpack != null)
					{
						...
						RockBottomAPI.getNet().sendToServer(
								new PacketItemUpdate(player.getUniqueId(), "jetpackData", jetpackSlotData));
					}
				...
				if (lantern != null)
				{
					...
					if (lanternEnergy > 0)
					{
						for (int x = -5; x < 6; x++)
						{
							for (int y = -5; y < 6; y++)
							{
								...
								RockBottomAPI.getGame().getWorld().setArtificialLight(Util.floor(player.x) + x,
										Util.floor(player.y) + y, (byte) 30);
							}
						}
					...
					RockBottomAPI.getNet()
							.sendToServer(new PacketItemUpdate(player.getUniqueId(), "lanternData", lanternSlotData));
				}
			...
			if (event.world != null && RockBottomAPI.getNet().isThePlayer(event.player))
			{
				...
				if (data != null && !data.getBoolean("is_creative"))
				{
					...
					if (jetpack != null)
					{
						...
						font.drawString(0.2F, 0.7F,
								String.format(engine + FormattingCode.WHITE.toString() + " - " + hover), 0.0175F);
					}
				}
			}
			return EventResult.DEFAULT;
