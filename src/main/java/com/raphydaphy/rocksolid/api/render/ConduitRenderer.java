...
public class ConduitRenderer<T extends Tile> extends DefaultTileRenderer<T>
{
	...
	@Override
	public void render(IGameInstance game, IAssetManager manager, IGraphics g, IWorld world, Tile tile, TileState state,
			int x, int y, TileLayer layer, float renderX, float renderY, float scale, int[] light)
	{
		...
		{
			manager.getTexture(this.texture.addSuffix(".MASTER")).draw(renderX, renderY, scale, scale, light);
		} else
		{
		...
		for (ConduitSide side : ConduitSide.values())
		{
			if (RockSolidAPILib.getTileFromConduitSide(new Pos2(x, y), side, world) != null)
			{
				...
				{
					manager.getTexture(this.texture.addSuffix("." + side.toString())).draw(renderX, renderY, scale,
							scale, light);
				}
			}
		}
