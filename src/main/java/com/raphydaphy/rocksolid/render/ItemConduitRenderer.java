...
public class ItemConduitRenderer<T extends Tile> extends DefaultTileRenderer<T>
{
    ...
    public void render(IGameInstance game, IAssetManager manager, Graphics g, IWorld world, T tile, TileState state, int x, int y, TileLayer layer, float renderX, float renderY, float scale, Color[] light){
        ...
        /* If you are brave
        if (conduit instanceof TileEntityAllocator)
        {
        	TileEntityAllocator allocator = (TileEntityAllocator)conduit;
        	
        	if (allocator.getInventory().get(0)!= null)
        	{
        		if (allocator.getInventory().get(0).getItem() != null)
        		{
        			Item theItemToRender = allocator.getInventory().get(0).getItem();
        			if (allocator.getInventory().get(0).getItem() instanceof ItemTile)
        			{
        				manager.getTexture(RockSolidLib.makeRes(theItemToRender.getName().addPrefix("tiles.").getResourceName().toString())).drawWithLight(renderX + (scale/3), renderY-(scale/3), scale / 3, scale / 3, light);
        			}
        			else
        			{
        				manager.getTexture(RockSolidLib.makeRes(theItemToRender.getName().addPrefix("items.").getResourceName().toString())).drawWithLight(renderX + (scale/3), renderY+(scale/3), scale/3 , scale/3, light);
        			}
        		}
        	}
        }
