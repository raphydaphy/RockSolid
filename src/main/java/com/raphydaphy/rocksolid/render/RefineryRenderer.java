...
public class RefineryRenderer extends MultiTileRenderer<TileRefinery>
{
	...
	{
		UP(1, 1, 1, 2), LEFT_INPUT(0, 0, -1, 0), RIGHT_OUTPUT(2, 0, 3, 0), RIGHT_UP(2, 1, 2, 2), RIGHT_SIDE(2, 1, 3, 1), LEFT_SIDE(0, 1, -1, 1), LEFT_UP(0, 1, 0, 2);
	...
	@Override
	public void render(IGameInstance game, IAssetManager manager, Graphics g, IWorld world, TileRefinery tile, TileState state,
			int x, int y, TileLayer layer, float renderX, float renderY, float scale, Color[] light)
	{
		...
		if (refinery != null && refinery instanceof TileEntityRefinery)
		{
			...
			for (RefinerySide side : RefinerySide.values())
			{
				...
				{
					TileEntity adjacentTile = world.getTileEntity(mainPos.getX() + side.getOffset().getX(), mainPos.getY() + side.getOffset().getY());
					if (adjacentTile != null && (adjacentTile instanceof TileEntityFluidConduit || adjacentTile instanceof TileEntityEnergyConduit))
					{
						manager.getTexture(this.texture.addSuffix("." + side.toString() +"." + innerCoord.getX() + "." + innerCoord.getY())).drawWithLight(renderX, renderY, scale, scale, light);
					}
				}
			}
		}
