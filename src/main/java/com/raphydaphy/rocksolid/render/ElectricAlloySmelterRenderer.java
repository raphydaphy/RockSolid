...
public class ElectricAlloySmelterRenderer extends MultiTileRenderer<BlockElectricAlloySmelter>
{
	protected final Map<Pos2, IResourceName> texturesActive;
	
    public ElectricAlloySmelterRenderer(final IResourceName texture, final MultiTile tile) 
    {
        super(texture, tile);
    ...
    @Override
    public void render(final IGameInstance game, final IAssetManager manager, final Graphics g, final IWorld world, final BlockElectricAlloySmelter tile, final int x, final int y, final float renderX, final float renderY, final float scale, final Color[] light) 
    {
    	final int meta = world.getMeta(x, y);
        final Pos2 innerCoord = tile.getInnerCoord(meta);
        final Pos2 mainPos = tile.getMainPos(x, y, meta);
        final TileEntityElectricAlloySmelter tileEntity = world.getTileEntity(mainPos.getX(), mainPos.getY(), TileEntityElectricAlloySmelter.class);
        IResourceName tex;
