...
public class BlockNuclearReactor extends MultiTile
{
	...
	public boolean onInteractWith(IWorld world, int x, int y, TileLayer layer, double mouseX, double mouseY, AbstractEntityPlayer player)
	{
		...
		if (tile != null)
		{
			...
			{
				player.openGuiContainer(new GuiNuclearReactor(player, (TileEntityNuclearReactor)tile), new ContainerNuclearReactor(player, (TileEntityNuclearReactor)tile));
				return true;
			}
		}
		return false;
	...
	public void updateRandomlyForRendering(IWorld world, int x, int y, TileLayer layer, AbstractEntityPlayer player)
	{
		if (this.isMainPos(x, y, world.getState(x, y)))
		{
			for (int i =0; i < 30; i++)
			{
				RockBottomAPI.getGame().getParticleManager().addSmokeParticle(RockBottomAPI.getGame().getWorld(), x+ 0.25 + Util.RANDOM.nextFloat(), y + 0.5 + (Util.RANDOM.nextFloat() / 10),0, -0, RockBottomAPI.getGame().getWorldScale() * 0.0005f);
			}
		}
	}*/
